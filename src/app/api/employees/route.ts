    ...
    const employees = await prisma.employee.findMany({
      ...
      include: {
        ...
        _count: {
          select: {
            appointments: {
              where: { deletedAt: null, date: { gte: new Date() }, status: { in: ["PENDING", "CONFIRMED"] } },
            },
          },
        },
      },
...
export async function POST(request: Request) {
  try {
    ...
    if (email) {
      ...
      if (existingEmployee) {
        return NextResponse.json({ error: "Ya existe un empleado con este correo electrónico" }, { status: 409 });
      }
    }
    ...
    if (servicioIds && servicioIds.length > 0) {
      await prisma.employeeService.createMany({
        data: servicioIds.map((serviceId) => ({
